...
SQLITE_WRAPPER_INLINE db::expr::raw::raw(const string & e)
  : expr(e) {
}
...
SQLITE_WRAPPER_INLINE db::expr::connective::connective(string o, const base & e1_, const base & e2_)
  : op(o)
  , e1(e1_)
  , e2(e2_) {
}
...
SQLITE_WRAPPER_INLINE db::string db::expr::connective::str() const {
  string res =
    DB_TEXT("(") + e1.str() + DB_TEXT(") ") + op + DB_TEXT(" (") + e2.str() + DB_TEXT(")");
  return res;
}
...
SQLITE_WRAPPER_INLINE db::expr::or_::or_(const base & e1_, const base & e2_)
...
SQLITE_WRAPPER_INLINE db::expr::not_::not_(const base & _exp)
  : exp(_exp) {
}
...
SQLITE_WRAPPER_INLINE bool db::expr::oper::check_escape(const param_types & type_) {
  switch (type_) {
    ...
    case e_char:
    case e_blob:
    case e_date_time: return true; break;
    case e_null:
    case e_bool:
    case e_int:
    case e_unsigned:
    case e_long:
    case e_float:
    case e_double: return false; break;
  }
  return false;
}

SQLITE_WRAPPER_INLINE db::expr::oper::oper(const field & fld, const string & o, const string & d)
  : _field(fld)
  , op(o)
  , data(d)
...
SQLITE_WRAPPER_INLINE db::expr::oper::oper(const field & fld, const string & o, const field & f2)
  : _field(fld)
  , op(o)
  , data(f2.fullName())
  , escape(false) {
...
SQLITE_WRAPPER_INLINE db::expr::like::like(const field & fld, const string & d)
  : oper(fld, DB_TEXT("LIKE"), d) {
}

SQLITE_WRAPPER_INLINE db::expr::in::in(const field & fld, const string & set)
...
/// @date      20:2:2009   14:23
///
SQLITE_WRAPPER_INLINE db::expr::in operator<<(const db::field & fld, const db::char_type * f2) {
...
SQLITE_WRAPPER_INLINE db::expr::and_
  operator&&(const db::expr::base & o1, const db::expr::base & o2) {
  return db::expr::and_(o1, o2);
}

/// overload from the global || operator
SQLITE_WRAPPER_INLINE db::expr::or_
  operator||(const db::expr::base & o1, const db::expr::base & o2) {
