...
SQLITE_WRAPPER_INLINE db::upd::upd(const string & t, const string & d)
  : table(t)
  , _where(DB_TEXT("True"))
  , _delim1(d)
  , _delim2(d) {
...
SQLITE_WRAPPER_INLINE db::upd::upd(const string & t, const string & d1, const string & d2)
  : table(t)
  , _where(DB_TEXT("True"))
  , _delim1(d1)
  , _delim2(d2) {
...
SQLITE_WRAPPER_INLINE db::upd & db::upd::set(const field & f, const string & val) {
  ...
  if (f.type() == e_string || f.type() == e_char || f.type() == e_blob || f.type() == e_date_time)
...
SQLITE_WRAPPER_INLINE db::string db::upd::str() const {
  ...
  split sets;
  for (size_t i = 0; i < fields.size(); i++) sets.push_back(fields[i] + DB_TEXT("=") + values[i]);
  q += sets.join(DB_TEXT(","));
  if (_where.length()) q += DB_TEXT(" WHERE ") + _where;
  return q;
}
