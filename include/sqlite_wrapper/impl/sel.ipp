///	@file		include/sqlite_wrapper/impl/sel.ipp
...
SQLITE_WRAPPER_INLINE
  db::sel::sel(const string & tablename, const string & delim1, const string & delim2)
  : _distinct(false)
  , _limit(0)
  , _offset(0)
  , _where(DB_TEXT("True"))
...
SQLITE_WRAPPER_INLINE db::sel & db::sel::result_no_delim(string r, string alias /*=DB_TEXT("")*/) {
  if (alias.length()) r += DB_TEXT(" AS ") + alias;
...
SQLITE_WRAPPER_INLINE db::sel & db::sel::result(string r, string alias /*=DB_TEXT("")*/) {
  if (alias.length()) r += DB_TEXT(" AS ") + alias;
  if (r != DB_TEXT("*")) detail::front_back_delim(r, _delim1, _delim2);
  _results.push_back(r);
  return *this;
}
...
SQLITE_WRAPPER_INLINE db::sel & db::sel::order_by(string ob, bool ascending /*=true*/) {
  ...
  if (!ascending) value += DB_TEXT(" DESC");
...
SQLITE_WRAPPER_INLINE db::sel::operator db::string() const {
  ostringstream res;
  res << DB_TEXT("SELECT ");
  if (_distinct) res << DB_TEXT("DISTINCT ");
  res << _results.join(DB_TEXT(","));
  res << DB_TEXT(" FROM ");
  res << _sources.join(DB_TEXT(","));

  if (
    _join.first.table().length() != 0 && _join.second.table().length() != 0 && _sources.size() == 1)
    res << DB_TEXT(" INNER JOIN ") << _join.first.table() << DB_TEXT(" ON ")
        << _join.first.fullName() << DB_TEXT(" = ") << _join.second.fullName() << DB_TEXT(" ");

  if (_where != DB_TEXT("True")) res << DB_TEXT(" WHERE ") << _where;
  if (_groupBy.size() > 0) res << DB_TEXT(" GROUP BY ") << _groupBy.join(DB_TEXT(","));
  if (_having.length()) res << DB_TEXT(" HAVING ") << _having;
  if (_orderBy.size() > 0) res << DB_TEXT(" ORDER BY ") << _orderBy.join(DB_TEXT(","));
  if (_limit) res << DB_TEXT(" LIMIT ") << string(detail::to_string(_limit));
  if (_offset) res << DB_TEXT(" OFFSET ") << string(detail::to_string(_offset));

  string ret(res.str());
  detail::replace_all(ret, _delim1 + _delim1, _delim1);
  if (_delim1 != _delim2) detail::replace_all(ret, _delim2 + _delim2, _delim2);
  return ret;
}
