...
db::sel::sel( const string& tablename ) : _distinct(false), _limit(0), _offset(0), 
_where(DB_TEXT("True")), _delim1(DB_TEXT("")), _delim2(DB_TEXT(""))
{
	source(tablename);
}

db::sel::sel( const string& tablename, const string& delim ) : _distinct(false), 
...
db::sel::sel( const string& tablename, const string& delim1, const string& delim2 ) : _distinct(false), 
...
db::sel::sel( const sel& s ) : _distinct(s._distinct), _limit(s._limit),
...
db::sel & db::sel::operator=( const sel& s )
{
...
db::sel & db::sel::operator,( const db::string& s )
{
	return this->result(s);
}

db::sel & db::sel::operator%( const expr::base& e )
{
	return this->where(e.str());
}

db::sel & db::sel::operator<( const field_pair& f )
{
	return this->join(f);
}

db::sel & db::sel::distinct( bool d )
{
	_distinct = d; 
	return *this;
}

db::sel & db::sel::limit( int value )
{
	_limit = value;	
	return *this;
}

db::sel & db::sel::join( const field_pair& f )
{
	_join = f;	
	return *this;
}

db::sel & db::sel::offset( int value )
{
	_offset = value; 
	return *this;
}

db::sel & db::sel::result_no_delim( string r, string alias/*=DB_TEXT("")*/ )
{
	if (alias.length())
		r += DB_TEXT(" AS ") + alias;
...
db::sel & db::sel::result( string r, string alias/*=DB_TEXT("")*/ )
{
	if (alias.length())
		r += DB_TEXT(" AS ") + alias;
	if (r!=DB_TEXT("*")) 
...
db::sel & db::sel::result_max( string r, string alias/*=DB_TEXT("")*/ )
{
	r = DB_TEXT("MAX(")+r;
	r += DB_TEXT(")");
	if (r!=DB_TEXT("*")) 
...
db::sel & db::sel::source( string s, string alias/*=DB_TEXT("")*/ )
{
...
db::sel & db::sel::where( const expr::base & w )
{
	_where = expr::and_(expr::raw(_where), w).str();	
	return *this;
}

db::sel & db::sel::where( string w )
{
	_where = expr::and_(expr::raw(_where), expr::raw(w)).str();
	return *this;
}

db::sel & db::sel::group_by( string gb )
{
...
db::sel & db::sel::having( const expr::base & h )
{
	_having = h.str(); 
	return *this;
}

db::sel & db::sel::having( string h )
{
	_having = h;
	return *this;
}

db::sel & db::sel::order_by( string ob, bool ascending/*=true*/ )
{
	string value = ob;
...
db::sel::operator db::string() const
{
	ostringstream res;
	res << DB_TEXT("SELECT ");
	if (_distinct)
