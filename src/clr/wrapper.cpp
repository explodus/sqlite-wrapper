...
public
enum class param_types : unsigned short {
...
public
enum class field_extra : unsigned short {
...
namespace detail {
  ...
  inline String ^ escape_sql(String ^ const str) {
    if (str == "NULL") return gcnew String(str);
...
/// splitmap basis
public
ref class split_map : public Dictionary< String ^, String ^ > {
public:
...
/// split base
public
ref class split : public List< String ^ > {
public:
  /// @brief        empty split
  ///
  /// <BR>qualifier
  ...
  split(List< String ^ > data) {
    ...
    this->Capacity = data.Capacity;
      for each(String ^ i in data) { Add(i); }
  }
  ...
  split(String ^ s_, String ^ delim_ /*=DB_TEXT(" ")*/) {
    ...
    for (std::vector< db::char_type * >::const_iterator i(pointers.begin()), e(pointers.end());
         i != e;
         ++i)
      data.push_back(db::string(*i));

    for (std::vector< db::string >::const_iterator i(data.begin()), e(data.end()); i != e; ++i)
  ...
  split ^
    slice(Int32 start, Int32 end) {
      ...
      for (int i = start; i < end; ++i) data->Add(this[i]);
      return data;
    }
...
public:
  /// @brief        field
  ///
  /// <BR>qualifier : _name(n) , _table(gcnew String("")) , _sourcename(gcnew String("")) , _type(t)
  /// , _extra(e_nullExtra) , _values(gcnew Values()) , _length(-1) <BR>access    public
  /// @return
  /// @param        String ^ n
  ...
  ///
  /// <BR>qualifier : _name(n) , _table(gcnew String("")) , _sourcename(gcnew String("")) ,
  /// _type(tp(val)) , _extra(e_nullExtra) , _values(gcnew Values()) , _length(-1) <BR>access public
  /// @return
  /// @param        String ^ n
  /// @param        T val
  ///
  ...
  /// @brief        field
  ///
  /// <BR>qualifier : _name(n_tbl->Key) , _table(n_tbl->Value) , _sourcename(gcnew String("")) ,
  /// _type(t) , _extra(e_nullExtra) , _values(vals) , _length(-1) <BR>access    public
  /// @return
  ...
  /// @brief        field
  ///
  /// <BR>qualifier : _name(f->_name) , _table(f->_table) , _sourcename(f->_sourcename) ,
  /// _type(f->_type) , _extra(f->_extra) , _values(f->_values) , _length(f->_length) <BR>access
  /// public
  /// @return
  /// @param        field ^ f
  ///
  ...
  ///
  /// <BR>qualifier : _name(gcnew String("")) , _table(gcnew String("")) , _sourcename(gcnew
  /// String("")) , _type(e_int) , _extra(e_nullExtra) , _values(gcnew Values()) , _length(-1)
  /// <BR>access    public
  /// @return
  ///
  /// @author       T. Schroeder (explodus@gmx.de)
  ...
  /// @date         22.10.2013 7:58
  ///
  inline String ^ table() { return _table; }
  ...
  /// @date         22.10.2013 7:58
  ///
  void set_sourcename(String ^ sourcename) { _sourcename = sourcename; }
  ...
  /// @date         22.10.2013 7:58
  ///
  bool has_extra(field_extra extra) { return ((_extra & static_cast< unsigned >(extra)) == 1); }
  ...
  void add_extra(field_extra extra) {
    if (!(_extra & static_cast< unsigned >(extra))) _extra |= static_cast< unsigned >(extra);
  }
  ...
  /// @date         22.10.2013 8:07
  ///
  bool operator==(field ^ const fd) { return fd->fullName() == fullName(); }
  ...
  /// @date         22.10.2013 8:07
  ///
  bool operator!=(field ^ const fd) { return !(*this == fd); }
};
...
public
  ...
  public:
    ...
    virtual String ^
      ...
      split
      ^
      getExtraTables() { return extraTables; }
...
public
  ...
  public:
    ...
    virtual String ^ str() override { return expr; }
  };

public
  ref class connective : public base_ {
  private:
    String ^ op;
  ...
  public:
    ...
    virtual String ^ str() override {
      String ^ res = gcnew String("(" + e1->str() + ") " + op + " (" + e2->str() + ")");
      return res;
    }
  };

public
  ref class and_ : public connective {
  public:
...
public
  ref class or_ : public connective {
  public:
...
public
  ...
  public:
    ...
    ///
    virtual String ^ str() override { return gcnew String("not (" + exp->str() + ")"); }
  };

  /// base class for operators in sql terms
public
  ref class oper : public base_ {
  protected:
    field ^ const _field;
    String ^ op;
    String ^ data;
    ...
    bool check_escape(param_types type_) {
      switch (type_) {
        ...
        case param_types::e_date_time: return true; break;
        ...
        case param_types::e_double: return false; break;
      }
      return false;
    }
    ...
    /// <BR>qualifier : _field(fld)*/, op(o)*/, data(DB_TEXT("0"))*/,
    /// escape(check_escape(_field.type())) <BR>access    protected
    ///
    /// @return
  ...
  public:
    ...
    virtual String ^ str() override {
      String ^ res = gcnew String("");
      res += _field->fullName() + " " + op + " " + (escape ? detail::escape_sql(data) : data);
      return res;
    }
  };
...
public
  ...
  public:
    ...
    ///
    template< class T >
    eq(field ^ const fld, T d)
      : oper(fld, gcnew String("="), d) {}
  };

  ///<> not equal operator
public
  ref class not_eq_ : public oper {
  public:
    ...
    ///
    template< class T >
    not_eq_(field ^ const fld, T d)
      : oper(fld, gcnew String("<>"), d) {}
  };

  ///> greater then operator
public
  ref class gt : public oper {
  public:
    ...
    ///
    template< class T >
    gt(field ^ const fld, T d)
      : oper(fld, gcnew String(">"), d) {}
  };

  /// greater then or equal operator
public
  ref class gt_eq : public oper {
  public:
    ...
    ///
    template< class T >
    gt_eq(field ^ const fld, T d)
      : oper(fld, gcnew String(">="), d) {}
  };

  /// lower then operator
public
  ref class lt : public oper {
  public:
    ...
    ///
    template< class T >
    lt(field ^ const fld, T d)
      : oper(fld, gcnew String("<"), d) {}
  };

  /// lower then or equal operator
public
  ref class lt_eq : public oper {
  public:
    ...
    ///
    template< class T >
    lt_eq(field ^ const fld, T d)
      : oper(fld, gcnew String("<="), d) {}
  };

  /// like operator
public
  ref class like : public oper {
  public:
    ...
    ///
    like(field ^ const fld, String ^ d)
      : oper(fld, gcnew String("LIKE"), d) {}
  };

  /// in Operator
public
  ref class in_ : public oper {
  public:
...
public:
  ...
  System::Object ^ Data() { return data_; } void Data(System::Object ^ val) { data_ = val; }
...
public
ref class row : ObservableCollection< param ^ > {
private:
public:
  row() {}
...
public:
  ...
  base_wrapper(String ^ db_file_name) {
    try {
      ...
      if (_base) _base->connect(marshal_as< std::string >(db_file_name), "");
#else
      if (_base) _base->connect(marshal_as< std::string >(db_file_name));
  ...
  ObservableCollection< row ^ > ^ execute(String ^ sql) {
    ...
    try {
      if (_base) q = _base->execute_ptr(marshal_as< std::wstring >(sql));
    ...
    if (!q->size()) return ret;

    for (db::query::const_iterator itb(q->begin()), ite(q->end()); itb != ite; ++itb) {
      const db::row & r = *itb;
      row ^ ref_r = gcnew row();

      for (db::row::const_iterator itrb(r.begin()), itre(r.end()); itrb != itre; ++itrb) {
        ...
        {  // param befüllen
          ...
          switch (p.get_type()) {
            case db::e_int:
            case db::e_unsigned: ref_p->Data(gcnew System::Int32(p.get_int())); break;
            case db::e_long: ref_p->Data(gcnew System::Int64(p.get_long())); break;
            case db::e_float:
            case db::e_double:
            case db::e_date_time: ref_p->Data(gcnew System::Double(p.get_double())); break;
            default: ref_p->Data(gcnew String(p.c_str())); break;
          }
        }
        ref_r->Add(ref_p);
      }
...
public:
  ...
  /// @ingroup			operators
  ///
  virtual sel ^ operator,(String ^ s) { return result(s, nullptr); }
  ...
  /// @ingroup			operators
  ///
  virtual sel ^ operator%(expr::base_ ^ e) { return where(e->str()); }
  ...
  /// @ingroup			operators
  ///
  virtual sel ^ operator<(FieldPair ^ f) { return join(f); }
  ...
  virtual sel ^
    ...
    virtual sel
    ^
    result_no_delim(String ^ r, String ^ alias /*=DB_TEXT("")*/) {
      if (alias && alias->Length) r += DB_TEXT(" AS ") + alias;
      _results->Add(r);
      return this;
    }
    ...
    virtual sel
    ^
    result(String ^ r, String ^ alias /*=DB_TEXT("")*/) {
      if (r != "*") r = detail::front_back_delim(r, _delim1, _delim2);
      if (alias && alias->Length) r += " AS " + detail::front_back_delim(alias, _delim1, _delim2);
      _results->Add(r);
      return this;
    }
    ...
    virtual sel
    ^
    result_max(String ^ r, String ^ alias /*=DB_TEXT("")*/) {
      if (r != "*") r = detail::front_back_delim(r, _delim1, _delim2);
  ...
  ///
  virtual sel ^
    source(String ^ s, String ^ alias /*=DB_TEXT("")*/) {
      s = detail::front_back_delim(s, _delim1, _delim2);

      if (alias && alias->Length) s += " AS " + detail::front_back_delim(alias, _delim1, _delim2);
    ...
    virtual sel
    ^
    order_by(String ^ ob, bool ascending /*=true*/) {
      ob = detail::front_back_delim(ob, _delim1, _delim2);
      if (!ascending) ob += " DESC";
      _orderBy->Add(ob);
      return this;
    }
    ...
    ^ str() {
        ...
        if (_distinct) res += "DISTINCT ";
        res += _results->join(",");
        res += " FROM ";
        res += _sources->join(",");

        if (
          _join && _join->Key->table()->Length != 0 && _join->Value->table()->Length != 0 &&
          _sources->Count == 1) {
          res += " INNER JOIN ";
          res += _join->Key->table();
          res += " ON ";
        ...
        if (_where != "True") res += " WHERE " + _where;
        if (_groupBy->Count > 0) res += " GROUP BY " + _groupBy->join(",");
        if (_having->Length) res += " HAVING " + _having;
        if (_orderBy->Count > 0) res += " ORDER BY " + _orderBy->join(",");
        if (_limit) res += " LIMIT " + System::Convert::ToString(_limit);
        if (_offset) res += " OFFSET " + System::Convert::ToString(_offset);

        if (_delim1->Length > 0) {
          res->Replace(_delim1 + _delim1, _delim1);
          if (_delim1 != _delim2) res->Replace(_delim2 + _delim2, _delim2);
        }
...
public:
  ...
  ins ^
    ...
    ins
    ^
    values(String ^ t, String ^ v, param_types p /*=e_int*/) {
    ...
    template< typename T >
    ins ^
    values(String ^ t, T v) { return values(t, System::Convert::ToString(v), tp(v)); }

    template<>
    ins ^
    values(String ^ t, Boolean v) {
      return values(t, System::Convert::ToString(v), tp(v));
    } template<>
    ins ^
    values(String ^ t, Int16 v) {
      return values(t, System::Convert::ToString(v), tp(v));
    } template<>
    ins ^
    values(String ^ t, Int32 v) {
      return values(t, System::Convert::ToString(v), tp(v));
    } template<>
    ins ^
    values(String ^ t, Double v) {
      return values(
        t,
    ...
    ins ^
    values(String ^ t, String ^ v) { return values(t, v, tp(v, false)); }
...
public:
  ...
  ///
  /// <BR>qualifier : table(t) , _where(gcnew String("True")) , _delim1(gcnew String("")) ,
  /// _delim2(gcnew String("")) <BR>access    public
  /// @return
  ...
  ///
  /// <BR>qualifier : table(t) , _where(gcnew String("True")) , _delim1(gcnew String(d)) ,
  /// _delim2(gcnew String(d)) <BR>access    public
  /// @return
  /// @param        String ^ t - should contain a table name
  ...
  ///
  /// <BR>qualifier : table(t) , _where(gcnew String("True")) , _delim1(gcnew String(d1)) ,
  /// _delim2(gcnew String(d2)) <BR>access    public
  /// @return
  /// @param        String ^ t - should contain a table name
  ...
  virtual upd ^
    ...
    upd
    ^
    set(field ^ f, String ^ val) {
      fields.Add(f->name());
      if (
        f->type() == param_types::e_string || f->type() == param_types::e_char ||
        f->type() == param_types::e_blob || f->type() == param_types::e_date_time)
    ...
    template< class T >
    upd ^
    set(field ^ f, T val) { return set(f, System::Convert::ToString(val)); }

    template<>
    upd ^
    set(field ^ f, Int32 val) { return set(f, System::Convert::ToString(val)); }

    template<>
    upd ^
    set(field ^ f, Double val) { return set(f, System::Convert::ToString(val)); }
    ...
    ^ str() {
        ...
        Int32 i = 0;
      for each(String ^ var in fields) { sets.Add(var + "=" + values[i++]); }

      q += sets.join(",");

      if (_where->Length) q += " WHERE " + _where;
...
inline clr_db::expr::in_::in_(clr_db::field ^ const fld, clr_db::sel ^ s)
  : clr_db::expr::oper(fld, gcnew String("IN"), gcnew String("(" + s->str() + ")")) {
}
